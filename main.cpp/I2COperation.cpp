...
*/
HM330XErrorCodes I2COperation::IIC_write_byte(uint8_t reg, uint8_t byte) {
    int ret = 0;
...
/**
    ...
    @return result of operation,non-zero if failed.
*/
HM330XErrorCodes I2COperation::IIC_write_16bit(uint8_t reg, uint16_t value) {
    int ret = 0;
    Wire.beginTransmission(_IIC_ADDR);
    Wire.write(reg);
...
/**
    ...
    @return result of operation,non-zero if failed.
*/
HM330XErrorCodes I2COperation::IIC_read_byte(uint8_t reg, uint8_t* byte) {
...
/**
    ...
    @return result of operation,non-zero if failed.
*/
HM330XErrorCodes I2COperation::IIC_read_16bit(uint8_t start_reg, uint16_t* value) {
    uint32_t time_out_count = 0;
    uint8_t val = 0;
    *value = 0;
    Wire.beginTransmission(_IIC_ADDR);
...
/**
    ...
    @return result of operation,non-zero if failed.
*/
HM330XErrorCodes I2COperation::IIC_read_bytes(uint8_t start_reg, uint8_t* data, uint32_t data_len) {
...
*/
void I2COperation::set_iic_addr(uint8_t IIC_ADDR) {
    _IIC_ADDR = IIC_ADDR;
}

HM330XErrorCodes I2COperation::IIC_SEND_CMD(uint8_t CMD) {
    Wire.beginTransmission(_IIC_ADDR);
    Wire.write(CMD);
